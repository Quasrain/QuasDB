...
namespace QuasDB
{
  ...
  public:
    AutoCompactTest()
    {
      ...
      tiny_cache_ = NewFIFOCache(100);
      options_.block_cache = tiny_cache_;
      DestroyDB(dbname_, options_);
      options_.create_if_missing = true;
      options_.compression = kNoCompression;
      EXPECT_THAT(DB::Open(options_, dbname_, &db_), QuasDB::test::IsOK());
    }
  ...
  void AutoCompactTest::DoReads(int n)
  {
    ...
    {
      ASSERT_THAT(db_->Put(WriteOptions(), Key(i), value), QuasDB::test::IsOK());
    }
    ASSERT_THAT(dbi->TEST_CompactMemTable(), QuasDB::test::IsOK());

    // Delete everything
    for (int i = 0; i < kCount; i++)
    {
      ASSERT_THAT(db_->Delete(WriteOptions(), Key(i)), QuasDB::test::IsOK());
    }
    ASSERT_THAT(dbi->TEST_CompactMemTable(), QuasDB::test::IsOK());
