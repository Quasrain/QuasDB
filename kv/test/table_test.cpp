...
#include "kv/db/dbformat.h"
#include "kv/db/memtable.h"
...
#include "kv/table/block_builder.h"
#include "kv/table/format.h"
...
namespace QuasDB
{
  ...
  TEST_F(Harness, Empty)
  {
    ...
    {
      Init(kTestArgList[i]);
      Random rnd(testing::UnitTest::GetInstance()->random_seed() + 1);
      Test(&rnd);
    }
  }
  ...
  TEST_F(Harness, SimpleSingle)
  {
    ...
    {
      Init(kTestArgList[i]);
      Random rnd(testing::UnitTest::GetInstance()->random_seed() + 2);
      Add("abc", "v");
      Test(&rnd);
    }
  }
  ...
  TEST_F(Harness, SimpleMulti)
  {
    ...
    {
      Init(kTestArgList[i]);
      Random rnd(testing::UnitTest::GetInstance()->random_seed() + 3);
      Add("abc", "v");
      Add("abcd", "v");
      Add("ac", "v2");
      Test(&rnd);
  ...
  TEST_F(Harness, SimpleSpecialKey)
  {
    ...
    {
      Init(kTestArgList[i]);
      Random rnd(testing::UnitTest::GetInstance()->random_seed() + 4);
      Add("\xff\xff", "v3");
      Test(&rnd);
    }
  }
  ...
  TEST_F(Harness, Randomized)
  {
    ...
    {
      Init(kTestArgList[i]);
      Random rnd(testing::UnitTest::GetInstance()->random_seed() + 5);
  ...
  static bool SnappyCompressionSupported()
  {
    ...
    size_t outlen;
    snappy::RawCompress(in.data(), in.size(), &(out)[0], &outlen);
    out.resize(outlen);
    return true;
  }
